...
function buildNav(){
 ...
 const u=getUser();
 ub.innerHTML = `<span>👤 ${u?u.username:''} ${u&&u.isAdmin?'(مدير)':''}</span><button class="btn secondary small" onclick="doLogout()">تبديل المستخدم / خروج</button>`;
}
function renderAll(){
...
function printElement(elId, title){
 ...
 const w = window.open('','_blank');
 w.document.write('<html dir="rtl"><head><title>'+title+'</title></head><body>'+printLetterhead(title)+el.innerHTML+'</body></html>');
...
function exportAllPdfPrint(){
 ...
 w.document.write('<html dir="rtl"><head><title>تقرير شامل</title></head><body>'+printLetterhead('تقرير النظام - '+todayStr())+pnl+'</body></html>');
 w.document.close(); w.print();
}
...
function deleteUser(id){ if(!confirm('تأكيد حذف المستخدم؟'))return; DB.users=DB.users.filter(x=>x.id!==id); saveDB(); renderAll(); }
