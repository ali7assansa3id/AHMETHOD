/* ============ CORE DATA & UTIL ============ */
const STORAGE_KEY='acc_system_data_v1';
const ALL_MODULES=[
...
const SUBSCRIPTION_PLANS={
 trial:{label:'تجريبي',days:14},
 basic:{label:'أساسي',days:30},
 professional:{label:'احترافي',days:365}
};
function isMasterAdmin(){ const u=getUser(); return !!(u && u.role==='masterAdmin'); }
function isCompanyAdmin(){ const u=getUser(); return !!(u && (u.role==='companyAdmin' || u.isAdmin) && !isMasterAdmin()); }
function canManageUsers(){ return isMasterAdmin() || isCompanyAdmin(); }
function getCurrentCompanyId(){ const u=getUser(); return u?u.companyId:null; }
function normalizeUsers(){
  if(!DB || !Array.isArray(DB.users)) return;
  let changed=false;
  DB.users.forEach(u=>{
    if(u.id===1 || u.username==='admin'){
      if(u.role!=='masterAdmin'){u.role='masterAdmin';changed=true;}
      if(u.companyId!=='MASTER'){u.companyId='MASTER';changed=true;}
      if(!u.subscription){u.subscription={plan:'MASTER',status:'active',startDate:todayStrSafe(),expiryDate:null};changed=true;}
    } else {
      if(!u.role) {u.role=u.isAdmin?'companyAdmin':'user';changed=true;}
      if(!u.companyId) {u.companyId='LEGACY';changed=true;}
      if(!u.subscription) {u.subscription={plan:'basic',status:'active',startDate:todayStrSafe(),expiryDate:null};changed=true;}
    }
  });
  if(changed) saveDB();
}
function subscriptionActive(u){
  if(!u) return false;
  if(u.role==='masterAdmin') return true;
  const sub=u.subscription||{};
  if(sub.status==='suspended') return false;
  if(sub.expiryDate && sub.expiryDate < todayStrSafe()) return false;
  return true;
}
function subscriptionText(u){
  if(!u || u.role==='masterAdmin') return 'إدارة النظام';
  const sub=u.subscription||{};
  const plan=(SUBSCRIPTION_PLANS[sub.plan]||{}).label || sub.plan || '-';
  return `${plan} — ${sub.expiryDate?'حتى '+sub.expiryDate:'مفتوح'}`;
}

function defaultData(){
 return {
  users:[{id:1,username:'admin',password:'admin123',isAdmin:true,role:'masterAdmin',companyId:'MASTER',companyName:'إدارة النظام',subscription:{plan:'MASTER',status:'active',startDate:todayStrSafe(),expiryDate:null}}],
  customers:[], suppliers:[],
...
function loadDB(){
 ...
 // fill in any missing keys (upgrades)
...
function postJournal(date, desc, debitAcc, debitAmt, creditAcc, creditAmt, sourceType, sourceId){
 if(Math.abs(debitAmt-creditAmt) > 0.011){ console.warn('قيد غير متوازن', debitAcc, debitAmt, creditAcc, creditAmt); }
 const num = nextCounter('jrn');
 DB.journal.push({id:uid(), number:num, date, desc, debitAcc, debitAmt:Number(debitAmt.toFixed(2)), creditAcc, creditAmt:Number(creditAmt.toFixed(2)), sourceType, sourceId});
 saveDB();
}
...
function doLogin(){
 ...
 if(!user){ document.getElementById('liErr').textContent='اسم المستخدم أو كلمة السر غير صحيحة'; return; }
 if(!subscriptionActive(user)){ document.getElementById('liErr').textContent='الاشتراك منتهي أو موقوف. تواصل مع مدير النظام.'; return; }
...
function registerSubscriber(){
 ...
 err.textContent='';
 if(!company||!username||!pass){err.textContent='أكمل بيانات الشركة واسم المستخدم وكلمة السر';return;}
 if(username.toLowerCase()==='admin'){err.textContent='اسم المستخدم admin محجوز لإدارة النظام';return;}
 if(pass.length<6){err.textContent='كلمة السر يجب ألا تقل عن 6 أحرف';return;}
 if(pass!==confirm){err.textContent='تأكيد كلمة السر غير مطابق';return;}
 if(DB.users.some(x=>x.username.toLowerCase()===username.toLowerCase())){err.textContent='اسم المستخدم مستخدم بالفعل';return;}
 const days=SUBSCRIPTION_PLANS[plan].days;
 const companyId='COMP-'+uid();
 const user={id:uid(),username,password:pass,isAdmin:true,role:'companyAdmin',companyId,companyName:company,perms:{},subscription:{plan,status:'active',startDate:todayStrSafe(),expiryDate:addDaysISO(days)}};
 DB.users.push(user);
 saveDB();
 document.getElementById('liUser').value=username;
 document.getElementById('liPass').value='';
 backToLogin();
 document.getElementById('liErr').style.color='var(--accent2)';
 document.getElementById('liErr').textContent='تم إنشاء الاشتراك. يمكنك الآن تسجيل الدخول.';
}
function doLogout(){
 logAudit('تسجيل خروج','system','-');
 DB.currentUserId=null; saveDB();
...
function showApp(){
 const u=getUser();
 if(!subscriptionActive(u)){ doLogout(); return; }
...
function buildNav(){
 ...
 ALL_MODULES.forEach(m=>{
  ...
  if(m.id==='users' && !canManageUsers()) return;
  if(m.id==='settings' && !isMasterAdmin()) return;
 ...
 const u=getUser();
 ub.innerHTML = `<span>👤 ${u?u.username:''} ${u&&u.isAdmin?'(مدير)':''}${u&&u.companyName?' — '+u.companyName:''}</span><button class="btn secondary small" onclick="doLogout()">تبديل المستخدم / خروج</button>`;
}
function renderAll(){
...
function saveSalesReturn(){
 const invId=Number(document.getElementById('srInv').value);
 const inv=DB.salesInvoices.find(x=>x.id===invId); if(!inv){toast('اختر فاتورة');return;}
 const date=document.getElementById('srDate').value;
 let total=0, retLines=[];
 inv.lines.forEach((l,idx)=>{
  const q=Number(document.getElementById('srQty_'+idx).value||0);
  if(q>0){
   const it=DB.items.find(x=>x.id==l.itemId);
   const unitPriceNet = (l.qty*l.price - computeDiscount(l.qty*l.price,'value',l.discVal||0))/l.qty;
   const lineTotal = unitPriceNet*q;
   total+=lineTotal;
   retLines.push({itemId:l.itemId, warehouseId:l.warehouseId, qty:q, unitPrice:unitPriceNet});
   const avgCost = getItemAvgCost(l.itemId) || (inv.cogs/inv.lines.reduce((s,x)=>s+x.qty,0));
   addStockBatch(l.itemId, l.warehouseId, q, avgCost, date, 'sales_return', invId);
 ...
 const number=nextCounter('sret');
 const ret={id:uid(), number, date, invoiceId:invId, invoiceNumber:inv.number, customerId:inv.customerId, lines:retLines, total};
 DB.salesReturns.push(ret); saveDB();
 postJournal(date,'مرتجع مبيعات #'+number, 'المبيعات (مرتجع)', total, 'عميل: '+getCustNameById(inv.customerId), total, 'sales_return', ret.id);
 logAudit('إضافة','sales','مرتجع مبيعات #'+number);
 closeModal(); renderAll(); toast('تم حفظ المرتجع');
}
function deleteSalesReturn(id){
 if(!requirePerm('sales','delete'))return;
 if(!confirm('تأكيد الحذف؟'))return;
 const r=DB.salesReturns.find(x=>x.id===id);
 r.lines.forEach(l=>{ consumeStockFIFO(l.itemId,l.warehouseId,l.qty,true); }); // reverse added stock
 DB.journal = DB.journal.filter(j=>!(j.sourceType==='sales_return'&&j.sourceId===id));
 DB.salesReturns=DB.salesReturns.filter(x=>x.id!==id); saveDB(); renderAll();
}
...
function reversePurchInvoice(id, silent){
 ...
 DB.stockBatches = DB.stockBatches.filter(b=>!(b.sourceType==='purchase' && b.sourceId===id));
 DB.stockOps = DB.stockOps.filter(o=> !(o.ref==='فاتورة مشتريات #'+inv.number));
 DB.journal = DB.journal.filter(j=>!(j.sourceType==='purchase'&&j.sourceId===id) && !(j.sourceType==='purchase_discount'&&j.sourceId===id));
 DB.otherIncome = DB.otherIncome.filter(o=>!(o.refType==='purchase'&&o.refId===id));
 if(!silent){ DB.purchaseInvoices=DB.purchaseInvoices.filter(x=>x.id!==id); saveDB(); }
}
...
function savePurchReturn(){
 const invId=Number(document.getElementById('prInv').value); const inv=DB.purchaseInvoices.find(x=>x.id===invId); if(!inv){toast('اختر فاتورة');return;}
 const date=document.getElementById('prDate').value; let total=0, retLines=[];
 inv.lines.forEach((l,idx)=>{
  const q=Number(document.getElementById('prQty_'+idx).value||0);
  if(q>0){
   const r=consumeStockFIFO(l.itemId,l.warehouseId,q,true);
   total += r.cost; retLines.push({itemId:l.itemId,warehouseId:l.warehouseId,qty:q});
   logStockOp(l.itemId, l.warehouseId, 'مرتجع مشتريات', 0, q, date, 'مرتجع مشتريات لفاتورة #'+inv.number);
  }
 });
 if(!retLines.length){toast('حدد كمية مرتجعة');return;}
 const number=nextCounter('pret');
 const ret={id:uid(),number,date,invoiceId:invId,invoiceNumber:inv.number,supplierId:inv.supplierId,lines:retLines,total};
 DB.purchaseReturns.push(ret); saveDB();
 postJournal(date,'مرتجع مشتريات #'+number,'مورد: '+getSuppNameById(inv.supplierId),total,'المشتريات (مرتجع)',total,'purchase_return',ret.id);
 logAudit('إضافة','purchases','مرتجع مشتريات #'+number); closeModal(); renderAll(); toast('تم الحفظ');
}
function deletePurchReturn(id){
 if(!requirePerm('purchases','delete'))return; if(!confirm('تأكيد الحذف؟'))return;
 const r=DB.purchaseReturns.find(x=>x.id===id);
 r.lines.forEach(l=> addStockBatch(l.itemId,l.warehouseId,l.qty, getItemAvgCost(l.itemId), todayStr(),'purchase_return_reverse', id));
 DB.journal=DB.journal.filter(j=>!(j.sourceType==='purchase_return'&&j.sourceId===id));
 DB.purchaseReturns=DB.purchaseReturns.filter(x=>x.id!==id); saveDB(); renderAll();
}
...
function importBackup(){
 ...
 inp.onchange=()=>{
  ...
  const reader=new FileReader();
  reader.onload=e=>{
   if(!confirm('سيتم استبدال كل البيانات الحالية بالنسخة الاحتياطية. متابعة؟')) return;
   try{ DB=JSON.parse(e.target.result); saveDB(); logAudit('استيراد','system','استعادة نسخة احتياطية'); toast('تم الاستيراد بنجاح'); showApp(); }
...
RENDERERS.users = function(root){
 if(!canManageUsers()){ root.innerHTML='<div class="empty">هذا القسم مخصص لمدير الشركة</div>'; return; }
 const me=getUser();
 const list=isMasterAdmin()?DB.users:DB.users.filter(u=>u.companyId===me.companyId);
 ...
 <div class="tableWrap"><table><thead><tr><th>اسم المستخدم</th><th>الشركة</th><th>النوع</th><th>الاشتراك</th><th>الحالة</th><th></th></tr></thead><tbody>
 ${list.map(u=>`<tr><td>${u.username}</td><td>${u.companyName||'-'}</td><td>${u.role==='masterAdmin'?'مدير النظام':u.role==='companyAdmin'?'مدير الشركة':'مستخدم'}</td><td>${subscriptionText(u)}</td><td>${subscriptionActive(u)?'<span class="tag paid">نشط</span>':'<span class="tag unpaid">موقوف/منتهي</span>'}</td>
 <td>${u.role!=='masterAdmin'?`<button class="linkBtn" onclick="openUserForm(${u.id})">تعديل</button> ${u.id!==1?`<button class="linkBtn" style="color:var(--danger)" onclick="deleteUser(${u.id})">حذف</button>`:''}`:''}</td></tr>`).join('')}
 </tbody></table></div></div>
...
function openUserForm(id){
 if(!canManageUsers()) return toast('ليس لديك صلاحية إدارة المستخدمين');
 const existing=id?DB.users.find(x=>x.id===id):null;
 if(existing && !isMasterAdmin() && existing.companyId!==getCurrentCompanyId()) return toast('لا يمكنك تعديل مستخدم خارج شركتك');
 const u=existing||{username:'',password:'',perms:{},subscription:{plan:'basic',status:'active',expiryDate:addDaysISO(30)},role:'user'};
 const roleOptions=isMasterAdmin()?`<option value="user" ${u.role==='user'?'selected':''}>مستخدم</option><option value="companyAdmin" ${u.role==='companyAdmin'?'selected':''}>مدير شركة</option>`:`<option value="user" selected>مستخدم</option>`;
 openModal(id?'تعديل المستخدم: '+u.username:'مستخدم جديد', `
 ${!id?`<div class="grid2"><div class="field"><label>اسم المستخدم</label><input id="uUsername"></div><div class="field"><label>كلمة السر</label><input id="uPassword" type="password"></div></div>`:''}
 <div class="grid2"><div class="field"><label>نوع المستخدم</label><select id="uRole">${roleOptions}</select></div><div class="field"><label>حالة الحساب</label><select id="uSubStatus"><option value="active" ${u.subscription?.status==='active'?'selected':''}>نشط</option><option value="suspended" ${u.subscription?.status==='suspended'?'selected':''}>موقوف</option></select></div></div>
 <div class="grid2"><div class="field"><label>نوع الاشتراك</label><select id="uPlan"><option value="trial" ${u.subscription?.plan==='trial'?'selected':''}>تجريبي</option><option value="basic" ${u.subscription?.plan==='basic'?'selected':''}>أساسي</option><option value="professional" ${u.subscription?.plan==='professional'?'selected':''}>احترافي</option></select></div><div class="field"><label>تاريخ انتهاء الاشتراك</label><input id="uExpiry" type="date" value="${u.subscription?.expiryDate||addDaysISO(30)}"></div></div>
 ...
 ${MODULE_PERM_LIST.map(m=>{ const p=(u.perms&&u.perms[m])||{}; const label=ALL_MODULES.find(x=>x.id===m).label; return `<tr><td>${label}</td><td><input type="checkbox" id="perm_${m}_view" ${p.view?'checked':''}></td><td><input type="checkbox" id="perm_${m}_add" ${p.add?'checked':''}></td><td><input type="checkbox" id="perm_${m}_edit" ${p.edit?'checked':''}></td><td><input type="checkbox" id="perm_${m}_delete" ${p.delete?'checked':''}></td></tr>`;}).join('')}
 </tbody></table>
...
function saveUser(id){
 if(!canManageUsers()) return toast('ليس لديك صلاحية');
 const perms={}; MODULE_PERM_LIST.forEach(m=>{ perms[m]={view:document.getElementById('perm_'+m+'_view').checked,add:document.getElementById('perm_'+m+'_add').checked,edit:document.getElementById('perm_'+m+'_edit').checked,delete:document.getElementById('perm_'+m+'_delete').checked}; });
 const role=document.getElementById('uRole').value, status=document.getElementById('uSubStatus').value, plan=document.getElementById('uPlan').value, expiry=document.getElementById('uExpiry').value;
 if(id){
  const u=DB.users.find(x=>x.id===id); if(!u || (!isMasterAdmin()&&u.companyId!==getCurrentCompanyId())) return toast('مستخدم غير متاح');
  u.perms=perms; u.role=isMasterAdmin()?role:'user'; u.isAdmin=u.role==='companyAdmin'||u.role==='masterAdmin'; u.subscription={...(u.subscription||{}),plan,status,expiryDate:expiry};
 } else {
  ...
  if(password.length<6){toast('كلمة السر يجب ألا تقل عن 6 أحرف');return;}
  if(DB.users.find(x=>x.username.toLowerCase()===username.toLowerCase())){toast('اسم المستخدم مستخدم بالفعل');return;}
  const me=getUser();
  DB.users.push({id:uid(),username,password,isAdmin:role==='companyAdmin',role,companyId:isMasterAdmin()?'MASTER':me.companyId,companyName:isMasterAdmin()?'إدارة النظام':me.companyName,perms,subscription:{plan,status,startDate:todayStrSafe(),expiryDate:expiry||addDaysISO(SUBSCRIPTION_PLANS[plan].days)}});
 }
 saveDB(); logAudit('إضافة/تعديل','users','مستخدم وصلاحيات'); closeModal(); renderAll(); toast('تم الحفظ');
}
function deleteUser(id){ if(!canManageUsers())return; const u=DB.users.find(x=>x.id===id); if(!u||u.role==='masterAdmin')return; if(!isMasterAdmin()&&u.companyId!==getCurrentCompanyId())return; if(!confirm('تأكيد حذف المستخدم؟'))return; DB.users=DB.users.filter(x=>x.id!==id); saveDB(); logAudit('حذف','users','مستخدم'); renderAll(); }
...
document.addEventListener('DOMContentLoaded', ()=>{
 ...
 document.getElementById('liUser').addEventListener('keydown', e=>{ if(e.key==='Enter') doLogin(); });
 if(DB.currentUserId){ showApp(); }
});
