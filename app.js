...
/* STORAGE_KEY was a single global constant in the original single-tenant file.
   ...
   set by loadDB(companyId) below. The original key name/prefix is preserved
   exactly so any pre-existing single-tenant data is never orphaned (see
   migrateLegacyDataIfNeeded() in js/auth.js, which moves old data under this
   same prefix into a proper company record on first run). */
let STORAGE_KEY='acc_system_data_v1';
const ALL_MODULES=[
...
function loadDB(companyId){
 /* companyId: when provided (multi-tenant flow), points STORAGE_KEY at that
    company's own isolated data before loading — this is the ONLY change to
    this function's behavior; everything else (defaults, upgrades, migration)
    is identical to the original single-tenant version. */
 if(companyId){ STORAGE_KEY = 'acc_system_data_v1__'+companyId; }
 try{
...
function doLogin(){
 ...
 if(!user){ document.getElementById('liErr').textContent='بيانات الدخول غير صحيحة'; return; }
...
function doLogout(){
 ...
 /* multi-tenant layer (js/auth.js) also keeps a platform-level session
    (super_admin / which company). Clear it too so Back-button / reload
    can't re-enter the app without a fresh login. Guarded so app.js has
    zero hard dependency on auth.js. */
 if(window.clearPlatformSession) window.clearPlatformSession();
}
function showApp(){
...
function buildNav(){
 ...
 const u=getUser();
 const compName = (DB.settings && DB.settings.companyName) ? DB.settings.companyName : '';
 ub.innerHTML = `${compName?`<span>🏢 ${compName}</span>`:''}<span>👤 ${u?u.username:''} ${u&&u.isAdmin?'(مدير الشركة)':''}</span><button class="btn secondary small" onclick="doLogout()">تبديل المستخدم / خروج</button>`;
}
function renderAll(){
...
function saveUser(id){
 ...
 if(id){
  ...
  /* Multi-tenant: usernames are unique across the WHOLE platform (login screen
     has no "company" field), not just within this company. js/users.js keeps
     a platform-wide index in sync; guarded so app.js still works standalone. */
  if(window.isUsernameTakenGlobally && window.isUsernameTakenGlobally(username)){ toast('اسم المستخدم مستخدم في النظام بالفعل'); return; }
 ...
 if(window.syncUserIndexForCurrentCompany) window.syncUserIndexForCurrentCompany();
}
function deleteUser(id){
 if(!confirm('تأكيد حذف المستخدم؟'))return;
 DB.users=DB.users.filter(x=>x.id!==id); saveDB(); renderAll();
 if(window.syncUserIndexForCurrentCompany) window.syncUserIndexForCurrentCompany();
}
